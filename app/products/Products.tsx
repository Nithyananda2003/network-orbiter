...
const ProductDetailModal: React.FC<{
  ...
  const isDefaultDescription = !productDetails[modelName]
  const details = productDetails[modelName] || {
    description: tableData 
      ? `The ${modelName} is a high-performance ${tableData.row[1]?.toLowerCase() || "networking component"} designed for carrier-grade deployments. It features ${tableData.row[3] || "advanced technical specifications"} and is optimized for ${tableData.row[2] || "professional network environments"}.`
      : "Detailed specifications for this product are currently being updated in our digital catalog. Please contact technical support for immediate documentation.",
    features: ["Carrier-grade reliability", "Network Orbiter verified quality", "Full technical support coverage"],
    specs: {}
  }

  // Combine table data with mapped specs, prioritizing table data for accuracy
  const combinedSpecs = { ...details.specs }
  if (tableData) {
    tableData.headers.forEach((header, index) => {
      if (index > 0) { // Skip model name which is usually index 0
        combinedSpecs[header] = tableData.row[index]
      }
    })
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
        <div className="flex items-center justify-between p-6 border-b border-border" style={{ backgroundColor: "rgba(34, 197, 94, 0.05)" }}>
          <div>
            <h3 className="text-2xl font-bold text-foreground" style={{ color: "#22c55e" }}>{modelName}</h3>
            <p className="text-sm text-muted-foreground">Product Preview & Details</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto scrollbar-thin">
          <div className="mb-8">
            ...
            <p className="text-foreground/90 leading-relaxed">{details.description}</p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-8">
            ...
            <div>
              <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-4">Technical Specs</h4>
              <div className="space-y-3">
                {Object.entries(combinedSpecs).length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground italic">Additional specifications pending update.</p>
                )}
              </div>
            </div>
          </div>
